...
interface IProfilePage extends IPage, IActionPage
{
	 ...
	 */
	public function SetAllowedActions($options,$username);
}
...
class ProfilePage extends ActionPage implements IProfilePage
{
	...
	public function SetAllowedActions($options,$username)
	{
		$allowPWD = $options->AllowPasswordChangeChecker($username);
		...
		$this->Set('AllowPWD', $allowPWD);
			
		if($allowPWD == false)
		{
			$this->Set('AllowOrganizationChange', $allowPWD);
			$this->Set('AllowPhoneChange', $allowPWD);
			$this->Set('AllowPositionChange', $allowPWD);
		}
		else
		{
