...
<table width="500" style="width: 500px">
	...
	<tr>
		...
		{if ($StartDate|date_format:"%Y-%m-%d") eq ($EndDate|date_format:"%Y-%m-%d")  }
			{$StartDate|date_format:"%Y-%m-%d %H:%M"} - {$EndDate|date_format:"%H:%M"}
		{else}
			{$StartDate|date_format:"%Y-%m-%d %H:%M"} - {$EndDate|date_format:"%Y-%m-%d %H:%M"}
		{/if}
		
		{if count($RepeatRanges) gt 0}
			<br/>
			<br/>
			The following repeated bookings will also be cancelled:
			{foreach from=$RepeatRanges item=date name=dates}
			{if ($StartDate) neq $date->GetBegin()}
				<br/>
				{$date->GetBegin()|date_format:"%Y-%m-%d"}
			{/if}
			{/foreach}
		{/if}
		</td>
