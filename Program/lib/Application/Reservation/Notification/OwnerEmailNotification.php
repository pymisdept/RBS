...
abstract class OwnerEmailNotification implements IReservationNotification
{
	...
	public function Notify($reservation)
	{
		...
		$d1 = $reservation->CurrentInstance()->EndDate()->ToTimezone($owner->Timezone());
		$d2 = date('Y-m-d H:i:s');		
		...
		{			
			if($d1 > $d2)
			{
				...
				Log::Error('Will send email');
			}
			else
			{
				Log::Error('Old message, no need to send');
			}
		}
		else
		{
			...
			Log::Error('Owner does not want these types of email notifications');
		}				
	}
